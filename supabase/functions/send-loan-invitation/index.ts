...
Deno.serve(async (req: Request) => {
  ...
  try {
    const { borrowerEmail, borrowerName, amount, lenderName }: LoanInvitation = await req.json();
    ...
    if (!resendApiKey) {
      ...
      return new Response(
        JSON.stringify({
          success: true,
          message: "Loan invitation logged (email service not configured)",
        }),
        {
          headers: {
            ...corsHeaders,
    ...
    const emailHtml = `
      <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
        ...
        <p>Click the button below to log in and review your loan:</p>
    ...
    return new Response(
      JSON.stringify({
        success: true,
        message: "Loan invitation email sent successfully",
        emailId: result.id,
      }),
      {
        headers: {
