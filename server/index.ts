...
function accessibleLoanRows(user: NonNullable<AuthRequest['authUser']>) {
...
function getLoan(id: string) {
  return db.prepare('SELECT * FROM loans WHERE id = ?').get(id) as Record<string, unknown> | undefined;
}
...
app.post('/api/auth/login', authLimiter, asyncRoute(async (req, res) => {
  ...
  const row = db.prepare('SELECT * FROM users WHERE email = ? AND registered = 1').get(email) as
...
app.post('/api/auth/signup', authLimiter, asyncRoute(async (req, res) => {
  ...
  const claim = db.transaction(() => {
    if (existing) {
      db.prepare(`
        UPDATE users SET full_name = ?, registered = 1, password_hash = ?, updated_at = ?
        WHERE id = ?
      `).run(fullName, passwordHash, now(), userId);
    } else {
      db.prepare(`
        INSERT INTO users (id, email, full_name, role, registered, password_hash, created_at)
        VALUES (?, ?, ?, 'borrower', 1, ?, ?)
      `).run(userId, email, fullName, passwordHash, now());
    }
    db.prepare(`
      UPDATE loans SET borrower_user_id = ?
      WHERE lower(borrower_email) = lower(?)
    `).run(userId, email);
  });
  claim();
  const user = publicUser(db.prepare('SELECT * FROM users WHERE id = ?').get(userId) as Record<string, unknown>);
...
app.post('/api/auth/forgot-password', authLimiter, asyncRoute(async (req, res) => {
  const email = normalizeEmail(req.body.email);
  const user = db.prepare('SELECT * FROM users WHERE email = ? AND registered = 1').get(email) as
    | Record<string, unknown>
    | undefined;
  if (user) {
    const token = createResetToken(String(user.id));
...
app.get('/api/loans', requireAuth, (req: AuthRequest, res) => {
  res.json(accessibleLoanRows(req.authUser!).map((row) => loanRow(row as Record<string, unknown>)));
});
...
app.post('/api/loans', requireRole('admin', 'master_admin'), asyncRoute(async (req, res) => {
  ...
  let borrower = db.prepare('SELECT id FROM users WHERE email = ?').get(borrowerEmail) as { id: string } | undefined;
  if (!borrower) {
    const id = newId();
    db.prepare(`
  ...
  res.status(201).json(loanRow(getLoan(id)!));
}));

app.patch('/api/loans/:id', requireRole('admin', 'master_admin'), (req: AuthRequest, res) => {
  ...
  res.json(loanRow(getLoan(loanId)!));
});

app.delete('/api/loans/:id', requireRole('admin', 'master_admin'), (req: AuthRequest, res) => {
  const loanId = String(req.params.id);
  const loan = getLoan(loanId);
  if (!loan) {
    res.status(404).json({ error: 'Loan not found' });
...
app.get('/api/repayments', requireAuth, (req: AuthRequest, res) => {
  const permittedIds = new Set(
    accessibleLoanRows(req.authUser!).map((row) => String((row as Record<string, unknown>).id)),
  );
...
app.post('/api/repayments', requireRole('admin', 'master_admin'), (req: AuthRequest, res) => {
  const loan = getLoan(String(req.body.loan_id));
  ...
  const row = db.prepare('SELECT * FROM repayments WHERE id = ?').get(id) as Record<string, unknown>;
  res.status(201).json(repaymentRow(row));
});

app.delete('/api/repayments/:id', requireRole('admin', 'master_admin'), (req: AuthRequest, res) => {
...
app.get('/api/users', requireRole('admin', 'master_admin'), (req: AuthRequest, res) => {
...
app.delete('/api/users/:id', requireRole('master_admin'), (req: AuthRequest, res) => {
  ...
  db.transaction(() => {
    db.prepare(`
      DELETE FROM loans
      WHERE borrower_user_id = ? OR lender_user_id = ? OR lower(borrower_email) = lower(?)
    `).run(userId, userId, user.email);
...
app.post('/api/users/:id/resend-invite', requireRole('admin', 'master_admin'), asyncRoute(async (req, res) => {
  const borrower = db.prepare('SELECT * FROM users WHERE id = ?').get(req.params.id) as
    | Record<string, unknown>
    | undefined;
  if (!borrower) {
...
app.get('/api/email-logs', requireRole('admin', 'master_admin'), (_req, res) => {
  res.json(db.prepare('SELECT * FROM email_logs ORDER BY sent_at DESC LIMIT 200').all());
});
